...
//保存初始化分片上传ID
let uploadId;
const multipartBucket = 'gonewlife2';
...
const putaoPng = require('../../assets/image/putao.jpeg');
  ...
  handleClick(e) {
    switch (e) {
      ...
      case 'appendObject':
        {
          ImagePicker.showImagePicker(options, response => {
            ...
            if (response.didCancel) {
              ...
              let source = {uri: response.uri};
              let position = 0;
              AliyunOSS.asyncAppendObject('luozhang002', 'xxx', source.uri, {
                appendPostion: `${position}`,
              }).then(res => {
      ...
      case 'resumeObject':
        {
          ImagePicker.showImagePicker(options, response => {
            ...
            if (response.didCancel) {
              ...
              let source = {uri: response.uri};
              AliyunOSS.asyncResumableUpload(
                'luozhang002',
                'zhongji',
                source.uri,
              ).then(res => {
      ...
      case 'initMultipartUpload':
        {
          AliyunOSS.initMultipartUpload(multipartBucket, multipartBucket)
            .then(e => {
      ...
      case 'listParts':
        {
          AliyunOSS.listParts(multipartBucket, multipartBucket, uploadId)
            .then(e => {
