...
class WeChatPage extends Component {
  ...
  componentDidMount() {
    let that = this;
    WeChat.registerApp('wx732379a9f9484d01').then(function(isSupport) {
  ...
  isWXAppSupportApi() {
    WeChat.isWXAppSupportApi().then(isSupportApi => {
      console.log(isSupportApi);
    });
  }
  getApiVersion() {
    WeChat.getApiVersion().then(apiVersion => {
      ...
      <ScrollView>
        <View style={{flex: 1, justifyContent: 'center', alignItems: 'center'}}>
          <View>
            ...
            />
            <Button title="获取API版本" onPress={() => this.getApiVersion()} />
            <Button title="打开微信" onPress={() => this.openWXApp()} />
            <Button title="微信认证" onPress={() => this.auth()} />
            <Button title="分享文本" onPress={() => this.shareText()} />
            <Button title="分享图片链接" onPress={() => this.shareImageUrl()} />
            <Button
              title="分享图片文件"
              onPress={() => this.shareImageFile()}
            />
            <Button
              title="分享图片资源"
