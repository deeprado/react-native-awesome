...
import {webUrl, IS_ANDROID} from '../../config';
import {EProfileRoutes} from '../../routes';
import {IPageProps} from '../../types/props';
import {THttpSuccessResponse} from '../../types/http';
...
@observer
export default class Profile extends Component<IAboutProps> {
  ...
  static navigationOptions = (config: NavigationScreenConfigProps) => {
    ...
    return {
      headerTitle: <CustomHeader title={i18n.t(LANGUAGE_KEYS.ABOUT)} />,
    };
  };

  @observable.ref
  private userInfo: IUserInfo = {
    gravatar: require('../../assets/image/gravatar.png'),
    name: '-',
    slogan: '-',
  };
  ...
  private fetchUserInfo() {
    fetch
      .get<IUserInfo>('/auth/admin')
      .then(
        action((userInfo: THttpSuccessResponse<IUserInfo>) => {
          this.userInfo = {
            ...userInfo.result,
  ...
  private fetchStatistic() {
    fetch
      .get<ITodayStatistic>('/expansion/statistic')
      .then(
        action((statistic: THttpSuccessResponse<ITodayStatistic>) => {
    ...
    return [
      {
        ...
        icon: (
          <FontAwesome
            name="stack-overflow"
            style={[
              obStyles.styles.lineIcon,
              fonts.h4,
              obStyles.styles.lineTitle,
            ]}
          />
        ),
    ...
    return [
      {
        ...
        icon: 'logo-github',
        onPress: () => {
          this.props.navigation.push(EProfileRoutes.Github);
        },
      },
      {
      ...
      },
      {
        ...
        icon: 'ios-mail',
        onPress: () =>
          this.openUrl('mailto:surmon@foxmail.com').catch(() =>
            Alert.alert(i18n.t(LANGUAGE_KEYS.CALL_EMAIL_ERROR)),
          ),
      },
    ];
  }
    ...
    return [
      {
        ...
        icon: 'ios-settings',
        onPress: () => {
          this.props.navigation.push(EProfileRoutes.Setting);
        },
      },
    ];
  }
    ...
    return (
      <View style={styles.container}>
        <ImageBackground
          ...
          blurRadius={90}>
          <Image style={styles.userGravatar} source={this.userInfo.gravatar} />
          <View>
        ...
        <View style={styles.statistic}>
          ...
          <View style={styles.statisticItem}>
            ...
            <Text style={styles.statisticTitle}>
              {i18n.t(LANGUAGE_KEYS.TODAY_VIEWS)}
            </Text>
          </View>
        </View>
        <View style={styles.section}>
          <SectionList
            ...
            ItemSeparatorComponent={() => (
              <View style={styles.lineItemSeparator} />
            )}
              ...
              <TouchableView
                ...
                <View style={styles.lineContent}>
                  {typeof item.icon === 'string' ? (
                    <Ionicon
                      style={[styles.lineIcon, styles.lineTitle]}
                      name={item.icon}
                    />
                  ) : (
                    item.icon
